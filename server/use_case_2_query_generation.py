...
def _get_information_context():
    with open('./data/data_catalog.txt', 'r') as file:
...
def _prompt_adjustment(user_input, information_context):
    structured_prompt = f"""
    You are a Language Model specializing in generating SQL queries based on user input. Below is the user's question and a data catalog of the hospital to assist you in crafting an appropriate SQL query.
    ...
    **Data Catalog Information**:
    ...
    Please provide the SQL query along with an explanation.
    Ensure that the SQL query uses only resources listed in the Data Catalog Information and is relevant to the user's input.
    """
...
def _call_LLM(prompt, temperature, top_p, top_k):
    ...
    try:
        ...
        return '### Use-Case 2: Query Generation\n\n' + processed_response
...
@app.route('/')
def hello_world():
    return '<h1>Use-Case #2  - Query Generation </h1>'
